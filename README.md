...
> [!CAUTION]  
> 拡張機能のidを設定しました。もしかしたら設定などがリセットされるかもしれません。  
> ご注意ください。
...
- この拡張機能はWebストアに公開していませんので、以下の手順でインストールしてください。
    1. [GitHub](https://github.com/ShigeUe/MentorCheckEx)からcloneする。  
    その時にブランチをmainにするとリリースバージョンになり、devにするとベータバージョンになります。
...
![](./images/f140067b8a57a156113c0d8c756db6a7.png)
    ...
    1. cloneしたフォルダの中のmentor_check_exフォルダを指定する。
以上でインストールできるはずです。
...
webpackとTypeScriptを使っているので以下の方法でビルドしてください。  
（nodejsのインストールは省きます）
...
## 変更履歴
- `1.0.9` 課題のデータの持ち方変更（将来の変更に対応）
- `1.0.8` レビューコードの取得のバグ修正
- `1.0.7` はじ副3でのカリキュラム調整に対応
- `1.0.6` 課題レビューDIFFに使うレビュー基準を同じカリキュラムに合わせた
- `1.0.5` 課題レビューDIFF機能の機能追加
- `1.0.4` どのコースにも紐づいていないGoogleドライブをはじ副に強制するボタンを設置。mergelyの高さを最大化するボタンを設置。
- `1.0.3` リファクタリング。Cloud9の情報を取得すると他の設定が初期化されるバグを修正。
- `1.0.0` TypeScriptに変更。使っていない機能を削除。
- `0.7.4` AWSの新サインインフォームに対応
- `0.7.3` manifest.jsonにkeyを指定し、IDが同じになるように設定
- `0.7.2` 課題レビューDIFF機能のデータの取得方法修正
- `0.7.1` Chromeのトラッキング保護が入っているとBASIC認証がうまく行かない点を修正
- `0.7.0` 課題レビューDIFF機能：サイドバーの表示オプション追加。検索機能追加。
- `0.6.9` slack新デザインの差異によるエラーを解消
- `0.6.8` 課題レビューDIFF機能：課題レビューURLの変更
- `0.6.7` 課題レビューDIFF機能で課題レビュー基準からコードを取得すると不完全になる点を修正
- `0.6.6` 課題レビューDIFF機能で比較元のコードをGitHubのコードか課題レビュー基準かを選べるように修正
- `0.6.5` 課題レビューDIFF機能で改行コードによる不具合があったのを修正
- `0.6.4` 課題レビューDIFF機能のファイルが無い場合でもレビューを継続できるようにした
- `0.6.3` 課題レビューDIFF機能のカリキュラム取得をGitHubのプライベートリポジトリ上のものに変更
